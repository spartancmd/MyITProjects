...
{
    std::string line{}; // line where the interpreted line from the file gets saved

    for(auto line_cnt = 1; line_cnt <= params.lines && getline(file, line); line_cnt++)
        std::cout << line << params.delimiter;
...
int main(int argc, char* argv[])
{
    ...
    Parameters params = process_cl_args(argc - 1, args); // args.length() - 1 because we don't need the file path that is at the end
