...
class Notebook:
    ...
    def get_redirect(self, command):
        ...
        if second_option == "names":
            return GetCommand.names(self.settings)
        elif second_option == "number":
            return GetCommand.number(self.settings, user_name)
        elif second_option == "age":
            return GetCommand.age(self.settings, user_name)
        elif second_option == "address":
            return GetCommand.address(self.settings, user_name)
        elif second_option == "personinfo":
            return GetCommand.person_info(self.settings, user_name)
    ...
    def command_process(self, command):
        if command[0] not in ("add", "get", "remove", "set", "help", "h", "quit", "q"): raise Command_not_valid(f"No such command {' '.join(command)}")
        ...
        if command[0] == "add":
            ...
            if AddCommand.is_valid(command, self.settings):
                ...
                return AddCommandResult(command, result)
            ...
            return HelpCommandResult(command, result)
            ...
            if SetCommand.is_valid(command, self.settings):
                ...
                return SetCommandResult(command, result)
        elif command[0] == "get":
            if GetCommand.is_valid(command, self.settings):
                ...
                return GetCommandResult(command, result)
            ...
            if RemoveCommand.is_valid(command, self.settings):
                ...
                return RemoveCommandResult(command, result)
